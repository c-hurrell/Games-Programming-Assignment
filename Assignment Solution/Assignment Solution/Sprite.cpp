...
{
	Vector2 windowSize = TextureManager::GetWindowSize();
	dest->x = (transform2D->posX ) - (transform2D->width / 2);
	dest->y = (transform2D->posY ) - (transform2D->height / 2); // -(TextureManager::origin.y * (transform2D->height * transform2D->scale) / 600);
